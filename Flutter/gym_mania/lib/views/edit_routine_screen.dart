...
class EditRoutineScreen extends StatefulWidget {
  ...
  EditRoutineScreen({Key? key, required this.routine}) : super(key: key);
...
class _EditRoutineScreenState extends State<EditRoutineScreen> {
  ...
  List<RoutineExercise> _originalRoutineExercises = [];
                            ...
                            children: [
                              ...
                              Obx(() => GestureDetector(
                                onTap: controller.isLoading.value ? null : () async {
                                  ...
                                  final success = await controller.updateRoutine(widget.routine, _originalRoutineExercises);
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final routineExercises = await _databaseService.getRoutineExercises(widget.routine.id!);

      final selectedExercises = <SelectedExercise>[];
      for (final routineEx in routineExercises) {
        ...
        selectedExercises.add(SelectedExercise(
          exercise: exercise,
          sets: routineEx.sets ?? 3,
          reps: routineEx.reps ?? 12,
          durationInSeconds: routineEx.duration ?? 60,
          restInSeconds: routineEx.restTime ?? 60,
        ));
      }

      controller.availableExercises.value = exercises;
      controller.initializeForEdit(widget.routine, selectedExercises);
      _originalRoutineExercises = routineExercises;
    } catch (e) {
  ...
  void _showExerciseSelectionDialog(BuildContext context) {
    Get.dialog(
      ExerciseSelectionDialog(
        ...
        selectedExercises: controller.selectedExercises.map((e) => e.exercise).toList(),
